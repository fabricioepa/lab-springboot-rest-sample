...
public class WebApiTest {
	...
	public void testCreateTicket() {
		ResponseEntity<TicketRest> response = createTicket();

		String path = response.getHeaders().getLocation().getPath();

		assertEquals(HttpStatus.CREATED, response.getStatusCode());
		assertTrue(path.startsWith("/api/v1/tickets"));
		TicketRest entity = response.getBody();
	...
	private ResponseEntity<TicketRest> createTicket() {
		HttpEntity<String> requestEntity = new HttpEntity<String>(read("test-data/ticket-sample.json"), TestContext.getHeaders());
		ResponseEntity<TicketRest> response = new RestTemplate().postForEntity(TestContext.path("/api/v1/tickets"),
				requestEntity, TicketRest.class);
		return response;
	}
